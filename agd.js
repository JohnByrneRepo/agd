  ...
  fs.readdir(path, function(err, list){
    ...
    list.forEach(function(item, idx){
      if(fs.lstatSync(path + '/' + item).isDirectory() &&
        ...
        getFolders(path + '/' + item, true);      // Or put the next step in a function and invoke it
      }
    });
    getFilesInFolders();
  });
...
var getFiles = function(path, key){
  fs.readdir(path, function(err, list){
    list.forEach(function(item){
      this.item = item;
      if(!fs.lstatSync(path + '/' + item).isDirectory()){
        if(folders[key].fileNames.length === 0 || folders[key].fileNames.indexOf(item) === -1){
          folders[key].fileNames.push(item);
          fs.readFile(folders[key].path + '/' + item, "utf-8", function read(err, data) {
            if (err) {
                throw err;
            }
            folders[key].files.push(data);
...
var generateMarkdown = function(){
  ...
  fs.writeFile(currentWorkingDirectory + '/' + outputFileName, JSON.stringify(folders, null, 4), function(err){
...
list.forEach(function(item){
    if(!fs.lstatSync(searchPath + '/' + item).isDirectory()){
      if(folders[key].fileNames.indexOf(item) === -1){
        folders[key].fileNames.push(item);
      }
    }
  });
